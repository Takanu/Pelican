...
- Spawner: A function that checks an update to see whether or not it matches given criteria in that function, returning a non-nil value if true.
...
public class SessionBuilder {
	...
	var setup: ((Pelican, SessionBuilder, Update) -> (Session))?
	...
	*/
	...
	/// The number of sessions the builder can have active at any given time.
	...
	- parameter spawner: A function that checks an update to see whether or not it matches given criteria in that function, returning a non-nil value if true.
		...
		if let id = spawner(update) {
			...
			// If not, build one
				...
				if setup != nil {
					...
					let session = setup!(bot, self, update)
					sessions[id] = session
					return session
				}
				...
				// If it doesn't, use the default initialiser.
					...
					let session = self.session.init(bot: bot, builder: self, update: update)
					session.postInit()
