...
final public class Sticker: TelegramType, SendType {
  ...
  public var set_name: String? // Name of the sticker set to which the sticker belongs
...
final public class StickerSet: TelegramType {
	public var storage = Storage()
	...
	public init(name: String, title: String, contains_masks: Bool, stickers: [Sticker]) {
		self.name = name
		self.title = title
	...
	public required init(row: Row) throws {
		...
		title = try row.get("title")
		contains_masks = try row.get("contains_masks")
		
		guard let stickersRow = row["stickers"] else { throw TGTypeError.ExtractFailed }
		self.stickers = try stickersRow.array?.map( { try Sticker(row: $0) } ) ?? []
	}
	...
	public func makeRow() throws -> Row {
		...
		try row.set("title", title)
		try row.set("contains_masks", contains_masks)
		try row.set("stickers", stickers)
...
final public class MaskPosition: TelegramType {
	...
	public required init(row: Row) throws {
		guard
			...
			else { throw TGTypeError.ExtractFailed }
